import { Card, Group, Text, Badge, Divider, ActionIcon, Tooltip } from "@mantine/core";
import { IconCheck, IconX, IconCopy, IconCheck as IconCheckMini } from "@tabler/icons-react";
    ...
    <>
      ...
      <Group justify="space-between">
        ...
        <Tooltip label={clipboard.copied ? "Đã copy!" : "Copy debug JSON"}>
          <ActionIcon
            variant="light"
            onClick={() => clipboard.copy(JSON.stringify(resp.debug, null, 2))}
            aria-label="Copy debug payload"
          >
            {clipboard.copied ? <IconCheckMini size={16} /> : <IconCopy size={16} />}
          </ActionIcon>
        </Tooltip>
      </Group>
